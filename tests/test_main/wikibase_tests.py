...
from tests.test_main.test_base.models import BigS, FieldsTest, Foo, Bar, DTModel
...
class WikibaseTest(TestCase):
    ...
    def test_mediawiki_version(self):
        version = connection.ops.mediawiki_version
        self.assertNotEqual(version, [])
...
class DatabaseSchemaTests(TransactionTestCase):
    def test_no_index_for_foreignkey(self):
        """
        Wikibase doesn't use indexes for foreign keys.
        """
...
class DateFieldTests(TestCase):
    def tests_date_interval(self):
        ...
        objs = FieldsTest.objects.filter(
            mod_date__gte=F('pub_date') + timedelta(days=3)).all()
...
class DateFunctionTests(TestCase):
    ...
    def test_trunc_func(self):
        ...
        with self.assertRaisesMessage(ValueError, msg):
            list(DTModel.objects.annotate(truncated=Trunc(
                'start_datetime', 'year', output_field=IntegerField())))

        with self.assertRaisesMessage(AssertionError, "'name' isn't a DateField, TimeField, or DateTimeField."):
            list(DTModel.objects.annotate(truncated=Trunc(
                'name', 'year', output_field=DateTimeField())))

        with self.assertRaisesMessage(ValueError, "Cannot truncate DateField 'start_date' to DateTimeField"):
            list(DTModel.objects.annotate(
                truncated=Trunc('start_date', 'second')))
        ...
        def test_date_kind(kind):
            self.assertQuerysetEqual(
                DTModel.objects.annotate(
                    truncated=Trunc('start_date', kind,
        ...
        def test_time_kind(kind):
            self.assertQuerysetEqual(
                DTModel.objects.annotate(
                    truncated=Trunc('start_time', kind,
        ...
        qs = DTModel.objects.filter(start_datetime__date=Trunc(
            'start_datetime', 'day', output_field=DateField()))
        self.assertEqual(qs.count(), 2)

    def test_trunc_time_func(self):
        start_datetime = microsecond_support(
            datetime(2015, 6, 15, 14, 30, 50, 321000))
        end_datetime = microsecond_support(
            datetime(2016, 6, 15, 14, 10, 50, 123000))
        if settings.USE_TZ:
            start_datetime = timezone.make_aware(start_datetime, is_dst=False)
            end_datetime = timezone.make_aware(end_datetime, is_dst=False)
        self.create_model(start_datetime, end_datetime)
        ...
        self.assertEqual(DTModel.objects.filter(
        ...
        with self.assertRaisesMessage(ValueError, "Cannot truncate DateField 'start_date' to TimeField"):
            list(DTModel.objects.annotate(truncated=TruncTime(
