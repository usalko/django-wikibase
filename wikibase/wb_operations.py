...
class DatabaseOperations(BaseDatabaseOperations):
    compiler_module = 'wikibase.wb_compilers'
    ...
    integer_field_ranges = {
        ...
        # since wikibase 3 AutoField
        'AutoField': (WbDatabase.INT_MIN, WbDatabase.INT_MAX),
        # and BigAutoField are supported
        'BigAutoField': (WbDatabase.LONG_MIN, WbDatabase.LONG_MAX),
    }
    ...
    def mediawiki_version(self):
        ...
        if not groups:
            raise ValueError(
                "Version string \"%s\" does not match expected format" % server_version)
        result = []
    ...
    def date_trunc_sql(self, lookup_type, field_name):
        if lookup_type == 'year':
            ...
            sql = "EXTRACT(year FROM %s)||'-'||EXTRACT(month FROM %s)||'-01'" % (
                field_name, field_name)
        elif lookup_type == 'day':
            sql = "EXTRACT(year FROM %s)||'-'||EXTRACT(month FROM %s)||'-'||EXTRACT(day FROM %s)" % (
    ...
    def last_insert_id(self, cursor, table, column):
        cmd_data = {**self._last_insert_id_data,
        ...
        if value is not None:
            ...
            if 'charset' in connection.get_connection_params():
                if connection.get_connection_params()['charset'] in charset_map:
                    db_charset = charset_map[connection.get_connection_params()[
                        'charset']]
            if db_charset:
    ...
    def convert_decimalfield_value(self, value, expression, connection, context = None):
        ...
        # val = utils.format_number(
        #     float(value), field.max_digits, field.decimal_places)
        val = str(value)
    ...
    def combine_duration_expression(self, connector, sub_expressions):
        ...
        if isinstance(timedelta, datetime.timedelta):
            ...
            unit = 'second'
            value = str((decimal.Decimal(timedelta) * sign) /
    ...
    def savepoint_create_sql(self, sid):
        cmd_data = {**self._savepoint_create_data, **
                    {'savepoint_id': self.quote_name(sid)}}
        return Cmd('savepoint_create', data=cmd_data)

    def savepoint_commit_sql(self, sid):
        cmd_data = {**self._savepoint_commit_data, **
                    {'savepoint_id': self.quote_name(sid)}}
        return Cmd('savepoint_commit', data=cmd_data)

    def savepoint_rollback_sql(self, sid):
        cmd_data = {**self._savepoint_rollback_data,
                    **{'savepoint_id': self.quote_name(sid)}}
        ...
        for model in model_list:
            ...
            for f in model._meta.many_to_many:
                if not f.rel.through:
                    ...
                    column_name = self.quote_name(f.column)
                    sequence_name = get_autoinc_sequence_name(
                        self, f.m2m_db_table())
                    output.append(reset_value_sql % {'sequence_name': sequence_name,
                                                     'column_name': column_name,
                                                     'table_name': table_name})
    ...
    def sequence_reset_sql(self, style, model_list):
        ...
        procedure_sql = '\n'.join([
            '%s %s' % (KEYWORD('CREATE PROCEDURE'),
                       TABLE('%(procedure_name)s')),
            KEYWORD('AS'),
            '%s %s %s;' % (
        ...
        for model in model_list:
            for f in model._meta.local_fields:
                if isinstance(f, models.AutoField):
                    ...
                    procedure_name = get_reset_procedure_name(
                        self, model._meta.db_table)
        ...
        for procedure in procedures:
            output.append('%s %s;' %
                          (KEYWORD('EXECUTE PROCEDURE'), TABLE(procedure)))
        for procedure in procedures:
            output.append('%s %s;' %
                          (KEYWORD('DROP PROCEDURE'), TABLE(procedure)))
    ...
    def drop_sequence_sql(self, table):
        ...
        cmd_data = {**self._drop_sequence,
                    **{'name': self.get_sequence_name(table)}}
        return Cmd('drop_sequence', data=cmd_data)
    ...
    def adapt_datetimefield_value(self, value):
        ...
        if timezone.is_aware(value):
            if settings.USE_TZ:
                ...
                raise ValueError(
                    "Wikibase backend does not support timezone-aware datetimes when USE_TZ is False.")
    ...
    def adapt_timefield_value(self, value):
        ...
        if timezone.is_aware(value):
            raise ValueError(
                "Wikibase backend does not support timezone-aware times.")
